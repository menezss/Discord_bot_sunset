...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Remove uma advertência específica ou todas as advertências de um usuário.')
    .addUserOption(opt => opt.setName('usuario').setDescription('O usuário').setRequired(true))
    .addIntegerOption(opt => opt.setName('numero').setDescription('Número da advertência a remover (use /advertencias para ver a lista)').setMinValue(1).setRequired(false))
  ...
  async execute(interaction) {
    ...
    if (!numero && !limparTudo) {
      return interaction.reply({
        embeds: [embed.aviso('Parâmetro Necessário', 'Informe o `numero` da advertência a remover ou use `limpar_tudo: true` para apagar todas.')],
        ephemeral: true,
      });
    }
    ...
    if (limparTudo) {
      ...
      if (total === 0) {
        return interaction.reply({
          embeds: [embed.info('Sem Advertências', `**${alvo.tag}** não possuía advertências.`)],
          ephemeral: true,
        });
      }
    ...
    if (!removida) {
      return interaction.reply({
        embeds: [embed.erro('Não Encontrada', `A advertência #${numero} não existe para **${alvo.tag}**. Use **/advertencias** para ver a lista.`)],
        ephemeral: true,
      });
    }
